...
// Auth
function getAuthToken() { return memoryAuthToken; }
function setAuthToken(token) { memoryAuthToken = String(token || ''); }
function getRefreshToken() { return memoryRefreshToken; }
function setRefreshToken(token) { memoryRefreshToken = String(token || ''); }
function getUser() { return getJson(STORAGE_KEYS.USER); }
function setUser(user) { setJson(STORAGE_KEYS.USER, user); }
function clearAuth() {
  memoryAuthToken = '';
  memoryRefreshToken = '';
  // Clean up legacy persisted tokens if they still exist.
