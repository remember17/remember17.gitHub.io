 ...
 *
 * Loaded with `defer` after fh-theme.js / fh-i18n.js.
 */
(function () {
  ...
  function bindCloseButton(nav) {
    var btn = nav.querySelector('[data-fh-close]');
    if (!btn || btn.__bound) return;
    btn.__bound = true;
    btn.addEventListener('click', function () {
      if (!isNative()) return;
      try {
        window.FiveHowNativeBridge.request('app.closeWebShell', null);
      } catch (_) { /* native shell owns the close fallback */ }
    });
  }
  ...
  function syncNavDivider(nav) {
    var scrollable = document.documentElement.scrollHeight > window.innerHeight + 1;
    nav.classList.toggle('is-scrolled', scrollable && window.scrollY > 0);
  }
  ...
  function init() {
    ...
    window.addEventListener('resize', function () { measure(nav); syncNavDivider(nav); });
    window.addEventListener('orientationchange', function () { measure(nav); });
    window.addEventListener('scroll', function () { syncNavDivider(nav); }, { passive: true });
